#!/bin/bash
#SBATCH -J tr_s2ipB_k3_617
#SBATCH -p batch
#SBATCH -N 1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:1
#SBATCH -o /finance_ML/zhanghaohan/stock_language_model/logs/run_train_pool0709_sentence_s2ip_B_k3_ep3_000617XSHE.out
#SBATCH --reservation=finai

# Variant B: trainable anchors + centered + separate projections; K=3 prepend; max 3 epochs.
...
python -u scripts/train_blankgpt2_sentence_preset_anchor_s2ip_txncomplete_single_day.py \
  ...
  --output-root "$ROOT/training_runs/pool_0709_0710_train0709_sentence_s2ip_B_trainable_center_sep_k3" \
  --epochs 3 \
  --patience 3 \
  --window-len 50 \
  --vocab-size 40560 \
