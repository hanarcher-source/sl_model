...
# -*- coding: utf-8 -*-
"""
VARIANT: sentence preset S2IP + **margin / separation** regularizer (v5-style).

- Same backbone as `train_blankgpt2_sentence_preset_anchor_s2ip_txncomplete_single_day.py`.
- **Align (unchanged):** `CE - lambda_align * mean(top-K cosine scores)` (same as stock k3 trainer).
- **Sep (added):** hinge on cosine score margins to discourage flat routing:
    * `k_vs_k1`: penalize `relu(margin - (s^{(K)} - s^{(K+1)}))` per batch mean.
    * `mean_in_out`: penalize `relu(margin - (mean_in_top_(K+1) - mean_outside)))`.
    * `none`: disable separation term.

Val / early stopping: **CE only** (no align / sep in val loss).

Do not treat this file as the frozen baseline; use `..._baseline_cosine.py` for that.
...
Regularization (training only):
  - align = mean(top-K cosine scores); `total_loss = CE - lambda_align * align + lambda_sep * sep`
  - sep = hinge margin (see module docstring); warmups via `--align-warmup-steps`, `--sep-warmup-steps`

Val / early stopping: CE only (no align / sep).
...
def main():
    ...
    print(
        f"[device] {device} | stock={stock} | sentence_preset_s2ip+MARGIN_SEP | topk={args.topk_anchors} "
        f"| sep_mode={args.sep_mode} sep_λ={args.sep_lambda} sep_m={args.sep_margin}"
    )
    ...
    run_meta = {
        ...
        "align_reg_s2ip_style": {
            ...
            "term": "CE - lambda_align * mean(topK_cosine_scores)",
        },
        "sep_reg_margin": {
            ...
            "term": "+ lambda_sep * relu(margin - gap) on cosine scores (train only; val CE-only)",
        },
        "trainer_variant": "sentence_preset_s2ip_margin_sep_v1",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, y in pbar:
            ...
            with autocast_ctx:
                ...
                sep_loss, sep_logs = separation_margin_loss(
                    scores,
                    int(args.topk_anchors),
                    str(args.sep_mode),
                    float(args.sep_margin),
                )
                total_loss = ce_loss - lam * align_mean + lam_sep * sep_loss
            ...
            epoch_tot_sum += (ce_loss - lam * align_mean + lam_sep * sep_loss).item()
            epoch_batches += 1
        ...
        print(
            f"\n[Epoch {epoch}] train_ce={train_ce:.4f} train_align={train_align:.4f} train_sep={train_sep:.4f} | "
            ...
            torch.save(
                {
                    ...
                    "config": run_meta,
                    "variant": "sentence_preset_s2ip_margin_sep",
                },
