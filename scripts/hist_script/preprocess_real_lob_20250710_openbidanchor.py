...
from utility.sim_helper_unified import process_lob_data_real_flow_open_anchor
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess real LOB flow with a fixed 09:31 L1 bid price anchor.")
    ...
    print(f"\n================ {day} | {stock} | openbidanchor ================")
    df_day, bin_record = process_lob_data_real_flow_open_anchor(
    ...
    cache_path = os.path.join(
        output_dir,
        f"final_result_for_merge_realflow_openbidanchor_{day}_{stock_tag}_{ts}.joblib",
    )
    joblib.dump(df_day, cache_path, compress=3)

    bin_record_path = os.path.join(
        output_dir,
        f"bin_record_realflow_openbidanchor_{day}_{stock_tag}_{ts}.json",
    )
    ...
    summary_path = os.path.join(
        output_dir,
        f"final_result_for_merge_realflow_openbidanchor_{day}_{stock_tag}_{ts}.json",
    )
