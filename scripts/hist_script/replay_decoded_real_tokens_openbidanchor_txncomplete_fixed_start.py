...
def _evaluate(exp_dir: str, real_ref_dir: str):
    ...
    logger.info("=" * 70)
    logger.info("decoded real-token open-anchor txn-complete evaluation")
    logger.info("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="Replay real token stream through fixed open-bid-anchor decoding with txn-complete events and evaluate it.")
    ...
    parser.add_argument(
        "--split-cancel-sides",
        action="store_true",
        help="Replay 6-way split-cancel tokens (must match preprocess + bin_record.split_cancel_sides).",
    )
    ...
    exp_tag = "openbidanchor_txncomplete_splitcancel" if args.split_cancel_sides else "openbidanchor_txncomplete"
    exp_name = f"fixed_start_decoded_real_tokens_{exp_tag}_{stock_tag}"
    ...
    if br_split != bool(args.split_cancel_sides):
        raise RuntimeError(
            f"bin_record.split_cancel_sides={br_split} but --split-cancel-sides={args.split_cancel_sides}; "
            "use matching preprocess output and flag."
        )
        ...
        raw_generation_rows.append(
            {
                ...
                "real_event_semantic": getattr(row, "EventSemantic", None),
                "token": int(token_id),
                "price_bin": int(pbin),
                "qty_bin": int(qbin),
    ...
    notes = {
        ...
        "conversion_caveats": [
            "Only tokenizable real events from the txn-complete schema are replayed directly.",
            "Price is decoded from a fixed 09:31:00 best-bid anchor in full ticks.",
            "Passive post tokens are rejected if the decoded price crosses the current opposite touch.",
            "Transaction-complete tokens remove liquidity directly from the specified resting side.",
            "Cancel side is inferred from current book occupancy at the decoded absolute price.",
