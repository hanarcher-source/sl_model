...
if [[ -z "${STOCK:-}" || -z "${TAG:-}" || -z "${TEMP:-}" || -z "${TOPK:-}" || -z "${SWEEP_TAG:-}" || -z "${K_PREPEND:-}" ]]; then
  echo "Missing env vars. Need: STOCK, TAG, TEMP, TOPK, SWEEP_TAG, K_PREPEND"
  exit 2
fi
...
CKPT=$(ls -t $ROOT/training_runs/pool_0709_0710_train0709_sentence_preset_s2ip_win50_k${K_PREPEND}/${TAG}/model_cache/*_best.pt 2>/dev/null | head -n 1 || true)

if [[ -z "$PROC" || -z "$BIN" || -z "$REF" || -z "$CKPT" ]]; then
  echo "Could not resolve required inputs for TAG=$TAG K_PREPEND=$K_PREPEND"
  echo "PROC=$PROC"
  echo "BIN=$BIN"
  echo "REF=$REF"
  echo "CKPT=$CKPT"
...
OUT=$ROOT/saved_LOB_stream/pool_0709_0710_eval_0710_model_variants/sentence_preset_s2ip_k${K_PREPEND}/${SWEEP_TAG}
mkdir -p "$OUT"
