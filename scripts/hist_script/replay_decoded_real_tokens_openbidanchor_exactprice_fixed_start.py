...
from eval_generated_stream import (  # noqa: E402
    ...
    _to_serializable,
    compute_metrics,
    inject_lobbench_style_overall,
    load_lobster_pair,
    make_plots,
)
from sim_helper_unified import (  # noqa: E402
    apply_event_to_book_open_anchor,
...
def _evaluate(exp_dir: str, real_ref_dir: str):
    ...
    logger.info("=" * 70)
    logger.info("decoded real-token open-anchor exact-price evaluation")
    logger.info("=" * 70)
    ...
    inject_lobbench_style_overall(metrics, logger)
...
def main():
    parser = argparse.ArgumentParser(description="Replay real token stream through open-anchor timing/size decoding but exact real prices, then evaluate it.")
    parser.add_argument("--stock", required=True)
    parser.add_argument("--processed-real-flow-path", required=True)
    parser.add_argument("--bin-record-path", required=True)
    parser.add_argument("--real-ref-dir", required=True)
    ...
    parser.add_argument("--use-exact-qty", action="store_true", help="Override decoded quantity with exact real row quantity.")
    args = parser.parse_args()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    stock_tag = args.stock.replace("_", "")
    exp_prefix = "fixed_start_decoded_real_tokens_openbidanchor_exactprice_exactqty" if args.use_exact_qty else "fixed_start_decoded_real_tokens_openbidanchor_exactprice"
    exp_name = f"{exp_prefix}_{stock_tag}"
    ...
    if not anchor_meta:
        raise RuntimeError(f"Missing fixed anchor metadata for stock={args.stock} in {args.bin_record_path}")
    ...
    for idx, row in enumerate(token_df.itertuples(index=False), start=0):
        ...
        pbin, qbin, ibin, sbin = decode_order_token(token_id, PRICE_BIN_NUM, QTY_BIN_NUM, INTERVAL_BIN_NUM, N_SIDE)
        ...
        ev.abs_price = float(original_price)
        if pd.isna(getattr(row, "Price_OpenBid_diff", np.nan)):
            ev.ticks_from_mid = int(np.rint((original_price - anchor_price) / 0.01))
        else:
            ev.ticks_from_mid = int(getattr(row, "Price_OpenBid_diff"))
        sampled_qty = int(ev.qty)
        if args.use_exact_qty:
            ev.qty = int(original_qty)

        cur_t_ms = int(ev.t_ms)
        action = apply_event_to_book_open_anchor(book, ev)
        ...
        raw_generation_rows.append(
            {
                ...
                "real_price_openbid_diff": None if pd.isna(getattr(row, "Price_OpenBid_diff", np.nan)) else int(getattr(row, "Price_OpenBid_diff")),
                "token": int(token_id),
                "price_bin": int(pbin),
                "qty_bin": int(qbin),
                ...
                "decoded_price_ticks_from_anchor": int(ev.ticks_from_mid),
                "sampled_qty_before_override": int(sampled_qty),
                "decoded_qty": int(ev.qty),
                "decoded_abs_price": float(ev.abs_price),
                "price_source": "exact_real_row",
                "qty_source": "exact_real_row" if args.use_exact_qty else "decoded_from_bin",
    ...
    notes = {
        ...
        "price_source": "exact_real_row",
        "qty_source": "exact_real_row" if args.use_exact_qty else "decoded_from_bin",
        "price_anchor": anchor_meta,
        "conversion_caveats": [
            "Only tokenizable real events are replayed directly (sides 49, 50, 99).",
            "Price is copied exactly from the processed real-flow row rather than decoded from the fixed open anchor bins.",
            "Cancel side is inferred from current book occupancy at the exact absolute price.",
            "Execution rows are reconstructed indirectly from aggressive crossing fills discovered by the simulator.",
            "Inter-arrival values are still regenerated from the saved empirical per-bin distributions in the bin record.",
    ...
    if args.use_exact_qty:
        notes["conversion_caveats"].insert(
            2,
            "Quantity is copied exactly from the processed real-flow row rather than decoded from the quantity bins.",
        )
    else:
        notes["conversion_caveats"].insert(
            2,
            "Quantity is still regenerated from the saved empirical per-bin distributions in the bin record.",
        )
    ...
    notes["key_reference_metrics"] = {
        ...
        for key in ["spread", "orderbook_imbalance", "log_inter_arrival_time", "ask_volume_touch", "bid_volume_touch", "ofi"]
    }
    with open(notes_json, "w", encoding="utf-8") as fh:
