...
if [[ -z "${STOCK:-}" || -z "${TAG:-}" || -z "${TEMP:-}" || -z "${TOPK:-}" || -z "${SWEEP_TAG:-}" || -z "${TRAIN_ROOT:-}" ]]; then
  echo "Missing env vars. Need: STOCK, TAG, TEMP, TOPK, SWEEP_TAG, TRAIN_ROOT"
  exit 2
fi
...
OUT=$ROOT/saved_LOB_stream/pool_0709_0710_eval_0710_model_variants/sentence_s2ip_AB_fixed_sampling/${SWEEP_TAG}
mkdir -p "$OUT"
