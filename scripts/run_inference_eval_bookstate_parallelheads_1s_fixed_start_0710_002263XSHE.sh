#!/bin/bash
#SBATCH -J ie_bsph_2263_710
#SBATCH -p batch
#SBATCH -N 1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:1
#SBATCH -o /finance_ML/zhanghaohan/stock_language_model/logs/run_inference_eval_bookstate_parallelheads_1s_fixed_start_0710_002263XSHE_%j.out
...
OUT="$ROOT/saved_LOB_stream/pool_0710_bookstate_eval_1s/bsph_T300_ctx60_start1000"
mkdir -p "$OUT"

if [[ -z "$DATA_JOB" || -z "$CKPT" || -z "$META" ]]; then
  echo "Missing input TAG=$TAG DATA_JOB=$DATA_JOB CKPT=$CKPT"; exit 3
fi
