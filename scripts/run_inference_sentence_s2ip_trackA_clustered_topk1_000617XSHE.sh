#!/bin/bash
#SBATCH -J ie_s2ip_tA1_617
#SBATCH -p batch
#SBATCH -N 1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:1
#SBATCH -o /finance_ML/zhanghaohan/stock_language_model/logs/run_inference_sentence_s2ip_trackA_clustered_topk1_000617XSHE_%j.out
...
TRAIN_ROOT=$ROOT/training_runs/pool_0709_0710_train0709_sentence_preset_s2ip_win50_trackA_clustered_topk1_routerdiag
CKPT=$(ls -t "${TRAIN_ROOT}/${TAG}/model_cache/"*_best.pt 2>/dev/null | head -n 1 || true)

OUT=$ROOT/saved_LOB_stream/pool_0709_0710_eval_0710_model_variants/sentence_preset_s2ip_trackA_clustered_topk1_routerdiag/sentence_s2ip_trackA_T10_k0_win50
mkdir -p "$OUT"
