...
# -*- coding: utf-8 -*-
"""
Pretrained GPT-2 + sentence anchors + S2IP routing + **gated cross-attention** into the window.

Same as the cosine sentence-S2IP baseline, plus **before** prepending top-K anchors:
  - Multi-head cross-attention where Q = order-token embeddings [B,T,H], K/V = anchor bank [A,H].
  - A **scalar gate per batch** g = sigmoid(w·query + b) with **b initialized negative** so the
    cross-attn residual is **off by default** (g≈0); training can open the gate if useful.
  - emb <- emb + g * CrossAttn(emb, anchors).

Then the usual hard top-K **prepended** prefix and GPT-2 forward (unchanged vs baseline).

Inference: checkpoint `variant` = `sentence_preset_s2ip_gated_crossattn`; see
`inference_replay_blankgpt2_openbidanchor_txncomplete_fixed_start.py`.
...
class OrderGPT2_SentencePresetS2IP_GatedCrossAttn(nn.Module):
    """
    Sentence anchors + optional gated cross-attn from window embeddings into anchor bank,
    then hard top-K prefix (same as baseline).
    """
    ...
    def __init__(
        ...
        *,
        ...
        hsz = int(hidden_size)
        nh = int(cross_attn_heads)
        if hsz % nh != 0:
            raise ValueError(f"hidden_size {hsz} must be divisible by cross_attn_heads {nh}")
        ...
        # Gate from pooled window embedding (same query signal as routing). Bias init negative -> sigmoid≈0 -> no CA residual by default.
        self.gate_proj = nn.Linear(hsz, 1)
    ...
    def forward(self, x: torch.Tensor, forced_topk_idx: torch.Tensor | None = None):
        ...
        # Gated cross-attn: attend from each time step to anchor keys; residual scaled by sigmoid(gate_proj(query)).
        bsz = emb.shape[0]
...
@torch.no_grad()
def _anchor_geometry_diag(model: OrderGPT2_SentencePresetS2IP_GatedCrossAttn) -> dict:
    """
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=12,
        help="MHA heads for anchor cross-attention (hidden size must be divisible by this).",
    )
    p.add_argument(
        "--gate-init-bias",
        type=float,
        default=-5.0,
        help="Initial bias on gate_proj so sigmoid(gate)≈0 (cross-attn residual off) until training shifts it.",
    )
    ...
    exp_name = (
        f"preGPT2sentS2IPgca_{day_tag}_txncomplete_{stock_tag}_win{args.window_len}"
        f"_k{args.topk_anchors}_mh{args.cross_attn_heads}"
    )
    ...
    print(f"[device] {device} | stock={stock} | sentence_preset_s2ip_gated_crossattn | topk={args.topk_anchors}")
    ...
    run_meta = {
        ...
        "sentence_anchors": {
            ...
            "separate_proj": bool(args.separate_proj),
            "gated_cross_attn": {
                "heads": int(args.cross_attn_heads),
                "gate_init_bias": float(args.gate_init_bias),
                "gate_from": "pooled_order_embedding_mean",
                "residual": "emb + sigmoid(gate_proj(query)) * cross_attn(emb, anchors)",
            },
        },
