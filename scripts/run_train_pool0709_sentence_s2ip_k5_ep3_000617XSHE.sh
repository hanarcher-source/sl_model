#!/bin/bash
#SBATCH -J tr_s2ip_k5_617
#SBATCH -p batch
#SBATCH -N 1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:1
#SBATCH -o /finance_ML/zhanghaohan/stock_language_model/logs/run_train_pool0709_sentence_s2ip_k5_ep3_000617XSHE.out
#SBATCH --reservation=finai

# Sentence preset anchors + S2IP align; top-K=5 prepend; max 3 epochs (pair with k3 batch).
...
python -u scripts/train_blankgpt2_sentence_preset_anchor_s2ip_txncomplete_single_day.py \
  ...
  --output-root "$ROOT/training_runs/pool_0709_0710_train0709_sentence_preset_s2ip_win50_k5" \
  --epochs 3 \
  --patience 3 \
  --window-len 50 \
  --vocab-size 40560 \
