...
------------------------------------
    ...
    metrics_summary.json
        includes lobbench_style_overall (mean/median/IQM over reference_comparison losses)
"""
...
_LOB_BENCH_CANDIDATES = [
    os.path.abspath(os.path.join(_HERE, '..', 'LOB_bench')),
    os.path.abspath(os.path.join(_HERE, '..', '..', 'LOB_bench')),
    os.path.abspath(os.path.join(_HERE, '..', '..', '..', 'LOB_bench')),
]
for _LOB_BENCH in _LOB_BENCH_CANDIDATES:
    if os.path.isdir(_LOB_BENCH):
        if _LOB_BENCH not in sys.path:
            sys.path.insert(0, _LOB_BENCH)
        break
else:
    raise ModuleNotFoundError(
        'Could not locate LOB_bench. Checked: ' + ', '.join(_LOB_BENCH_CANDIDATES)
    )
...
def _lb_is_finite_number(x) -> bool:
...
def _lb_iqm(vals):
    """Mean of values between the 25th and 75th percentiles (LOB-Bench IQM note)."""
    if not vals:
        return None
    s = sorted(vals)
    n = len(s)
...
def _lb_collect_reference_losses(metrics: dict):
    """Per-metric Wasserstein and L1-by-group losses from reference_comparison.metrics."""
    w = {}
    l1 = {}
    ref = metrics.get("reference_comparison") or {}
    m = ref.get("metrics") or {}
    if not isinstance(m, dict):
        return w, l1
    for metric_name, payload in m.items():
        if not isinstance(payload, dict):
            continue
        if "weighted_wasserstein" in payload and _lb_is_finite_number(payload.get("weighted_wasserstein")):
            w[metric_name] = float(payload["weighted_wasserstein"])
        elif _lb_is_finite_number(payload.get("wasserstein")):
            w[metric_name] = float(payload["wasserstein"])
        if _lb_is_finite_number(payload.get("l1_by_group")):
            l1[metric_name] = float(payload["l1_by_group"])
    return w, l1


def _lb_aggregate_losses(vals):
    xs = [float(x) for x in vals if _lb_is_finite_number(x)]
    if not xs:
...
def inject_lobbench_style_overall(metrics: dict, logger) -> None:
    """
    Add metrics['lobbench_style_overall'] and log LOB-Bench-style aggregates (mean / median / IQM)
    over finite Wasserstein and L1-by-group losses in reference_comparison.metrics.
    Matches the aggregation used in compute_overall_scores_lobbench_style.py.
    """
    w, l1 = _lb_collect_reference_losses(metrics)
    w_block = _lb_aggregate_losses(list(w.values()))
    l1_block = _lb_aggregate_losses(list(l1.values()))
    metrics["lobbench_style_overall"] = {
        "description": (
            "LOB-Bench-style aggregates: mean, median, and IQM across per-metric losses in "
            "reference_comparison.metrics (Wasserstein uses weighted_wasserstein when finite, "
            "else wasserstein; conditional metrics with null weighted_wasserstein are omitted)."
        ),
        "wasserstein": w_block,
        "l1_by_group": l1_block,
    }
    if not logger:
        return
    logger.info("\n── LOB-Bench-style overall aggregates (reference_comparison) ──")
    if w_block["n"] == 0:
        logger.info("  Wasserstein aggregate: n=0 (no finite comparative Wasserstein losses)")
    else:
        logger.info(
            "  Wasserstein aggregate: n=%d  mean=%.6g  median=%.6g  iqm=%.6g  (lower is better)",
            w_block["n"],
            w_block["mean"],
            w_block["median"],
            w_block["iqm"],
    ...
    if l1_block["n"] == 0:
        logger.info("  L1_by_group aggregate: n=0 (no finite L1-by-group losses)")
    else:
        logger.info(
            "  L1_by_group aggregate: n=%d  mean=%.6g  median=%.6g  iqm=%.6g  (lower is better)",
            l1_block["n"],
            l1_block["mean"],
            l1_block["median"],
            l1_block["iqm"],
